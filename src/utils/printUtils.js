export function printItem(itemData, itemType, centerLogo, centerName) {
  ...
  const savedCenterName = localStorage.getItem('scs_center_name') || '';
  const savedCenterLogo = localStorage.getItem('scs_center_logo') || '';
  const safeCenterName = (centerName && String(centerName).trim()) || savedCenterName || 'المركز';
  const safeCenterLogo = centerLogo || savedCenterLogo;
  
  // شعار المركز والعنوان
  const headerHTML = `
    <div style="text-align:center;padding:20px;border-bottom:2px solid #333;margin-bottom:20px;">
      ${safeCenterLogo ? `<img src="${safeCenterLogo}" style="height:70px;margin-bottom:10px;border-radius:8px;object-fit:contain;">` : '<div style="font-size:3rem;margin-bottom:10px;">🏥</div>'}
