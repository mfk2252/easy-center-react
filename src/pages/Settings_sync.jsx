          ...
          <div className="wg" style={{marginBottom:14}}>
            ...
            <div className="wg-b">
              <p style={{fontSize:'.86rem',color:'var(--g5)',marginBottom:16}}>
                إذا كانت بياناتك موجودة على جهاز معين ولا تظهر على أجهزة أخرى، استخدم هذه الأزرار للمزامنة الكاملة.
              </p>
              <div style={{display:'flex',gap:10,flexWrap:'wrap'}}>
                <button className="btn btn-p" onClick={async ()=>{
                  const { pushLocalToFirebase, getCenterId } = await import('../hooks/useStorage');
                  ...
                  await pushLocalToFirebase(centerId);
                  toast('✅ تم رفع كل البيانات لـ Firebase!','ok');
                }}>
                  ⬆️ رفع بياناتي لـ Firebase
                </button>
                <button className="btn btn-s" onClick={async ()=>{
                  ...
                  toast('✅ تم جلب كل البيانات!','ok');
                  setTimeout(()=>window.location.reload(), 1000);
                }}>
                  ⬇️ جلب بياناتي من Firebase
                </button>
              </div>
              <p style={{fontSize:'.75rem',color:'var(--g4)',marginTop:10}}>
                💡 <strong>رفع:</strong> من هذا الجهاز → Firebase<br/>
                💡 <strong>جلب:</strong> من Firebase → هذا الجهاز
              </p>
            </div>
          </div>
