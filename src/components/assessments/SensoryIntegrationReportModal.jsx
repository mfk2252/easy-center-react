  ...
  return (
    <div className="mbg" style={{ zIndex: 1150, display: 'flex', alignItems: 'center', justifyContent: 'center' }} onClick={(e) => e.target === e.currentTarget && onClose()}>
      <div className="mb" style={{ maxWidth: 1050, width: '96vw', maxHeight: '94vh', display: 'flex', flexDirection: 'column', padding: 0, overflow: 'hidden', borderRadius: 16 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            {onOpenIepBridge && (
              <button
                ...
                style={{ background: '#7c3aed', color: '#fff', fontWeight: 800, border: 'none' }}
              >
            ...
            <button
              ...
              style={{ background: '#25D366', color: '#fff', fontWeight: 800, border: 'none' }}
            >
              💬 مشاركة واتساب
            </button>
            ...
            <button
              ...
              className="btn btn-sm btn-g"
              onClick={onClose}
              style={{ color: '#cbd5e1', borderColor: 'rgba(255,255,255,0.2)', width: 32, height: 32, padding: 0 }}
            >
              ✕
            </button>
          </div>
        ...
        <div ref={reportRef} style={{ flex: 1, overflowY: 'auto', padding: '24px 30px', background: '#ffffff', color: '#1e293b' }}>
          ...
          <div style={{ borderBottom: '2px solid #0f172a', paddingBottom: 16, marginBottom: 20, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div style={{ textAlign: 'right' }}>
              <div style={{ fontSize: '.85rem', fontWeight: 800, color: '#475569' }}>المملكة العربية السعودية / جمهورية مصر العربية</div>
              <div style={{ fontSize: '.85rem', fontWeight: 800, color: '#475569' }}>مركز التأهيل الشامل والرعاية التخصصية</div>
              <div style={{ fontSize: '.78rem', color: '#64748b' }}>قسم العلاج الوظيفي وتأهيل التكامل الحسي والنمائي</div>
            </div>

            <div style={{ textAlign: 'center' }}>
              <div style={{ width: 52, height: 52, borderRadius: '50%', background: '#f1f5f9', border: '2px solid #cbd5e1', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.8rem', margin: '0 auto 4px' }}>
                🎯
              </div>
            ...
            <div style={{ textAlign: 'left' }}>
              <div style={{ fontSize: '.76rem', color: '#64748b' }}>رقم التقرير: <strong style={{ color: '#0f172a' }}>SI-{assessment.id?.slice(-6) || '2026'}</strong></div>
              <div style={{ fontSize: '.76rem', color: '#64748b' }}>تاريخ الفحص: <strong style={{ color: '#0f172a' }}>{assessment.date || new Date().toISOString().split('T')[0]}</strong></div>
              <div style={{ fontSize: '.76rem', color: '#64748b' }}>حالة التقرير: <strong style={{ color: '#059669' }}>معتمد رسمياً</strong></div>
            </div>
          </div>

          {/* ACADEMIC CITATION BADGE */}
          <div style={{ background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: 8, padding: '8px 14px', marginBottom: 18, fontSize: '.76rem', color: '#475569', display: 'flex', justifyContent: 'space-between', flexWrap: 'wrap', gap: 6 }}>
            <div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 10, background: '#f1f5f9', borderRadius: 10, padding: 14, marginBottom: 20 }}>
            <div>
              <span style={{ display: 'block', fontSize: '.72rem', color: '#64748b', fontWeight: 700 }}>اسم الطالب:</span>
              <strong style={{ fontSize: '.92rem', color: '#0f172a' }}>{assessment.studentName || '—'}</strong>
            </div>
            <div>
              <span style={{ display: 'block', fontSize: '.72rem', color: '#64748b', fontWeight: 700 }}>الأخصائي الفاحص:</span>
              <strong style={{ fontSize: '.92rem', color: '#0f172a' }}>{assessment.evaluator || 'أخصائي العلاج الوظيفي'}</strong>
            </div>
            <div>
              <span style={{ display: 'block', fontSize: '.72rem', color: '#64748b', fontWeight: 700 }}>المحك التشخيصي:</span>
              <strong style={{ fontSize: '.92rem', color: '#2563eb' }}>45 درجة (نصف المجموع)</strong>
            </div>
            <div>
              <span style={{ display: 'block', fontSize: '.72rem', color: '#64748b', fontWeight: 700 }}>نسبة الإنجاز:</span>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 12, marginBottom: 24 }}>
            ...
            <div style={{ background: '#ffffff', border: `2px solid ${scoreResult.severityColor}`, borderRadius: 12, padding: 14, textAlign: 'center', boxShadow: '0 4px 12px rgba(0,0,0,0.03)' }}>
              <div style={{ fontSize: '.74rem', color: '#64748b', fontWeight: 700 }}>الدرجة الكلية للمقياس</div>
              <div style={{ fontSize: '2rem', fontWeight: 900, color: scoreResult.severityColor, margin: '4px 0' }}>
                {scoreResult.totalRawScore} <span style={{ fontSize: '1rem', color: '#64748b' }}>/ 90</span>
              </div>
            ...
            <div style={{ background: '#ffffff', border: '1px solid #cbd5e1', borderRadius: 12, padding: 14, textAlign: 'center' }}>
              <div style={{ fontSize: '.74rem', color: '#64748b', fontWeight: 700 }}>التصنيف والتشخيص الإكلينيكي</div>
            ...
            <div style={{ background: '#ffffff', border: '1px solid #cbd5e1', borderRadius: 12, padding: 14, textAlign: 'center' }}>
              <div style={{ fontSize: '.74rem', color: '#64748b', fontWeight: 700 }}>المحاور المستوفاة</div>
              <div style={{ fontSize: '1.4rem', fontWeight: 900, color: '#0f172a', margin: '4px 0' }}>
                {scoreResult.subscales.filter(s => s.raw >= 6).length} <span style={{ fontSize: '.9rem', color: '#64748b' }}>/ 9 أبعاد</span>
              </div>
          ...
          <div style={{ marginBottom: 24 }}>
            <h4 style={{ fontSize: '.95rem', fontWeight: 900, color: '#0f172a', margin: '0 0 10px', display: 'flex', alignItems: 'center', gap: 6 }}>
            ...
            <div style={{ border: '1px solid #cbd5e1', borderRadius: 10, overflow: 'hidden' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '.82rem', textAlign: 'right' }}>
                <thead>
                  <tr style={{ background: '#f8fafc', borderBottom: '1px solid #cbd5e1', color: '#475569', fontWeight: 800 }}>
                ...
                <tbody>
                  {scoreResult.subscales.map((sub, idx) => (
                    <tr key={sub.id} style={{ borderBottom: '1px solid #e2e8f0', background: idx % 2 === 0 ? '#ffffff' : '#fcfcfd' }}>
                      <td style={{ padding: '8px 12px', textAlign: 'center', fontWeight: 700, color: '#64748b' }}>{sub.num}</td>
                      <td style={{ padding: '8px 12px' }}>
                        <div style={{ fontWeight: 800, color: '#0f172a' }}>{sub.name}</div>
                        <div style={{ fontSize: '.7rem', color: '#64748b' }}>{sub.englishName}</div>
                      </td>
                      ...
                      <td style={{ padding: '8px 12px', textAlign: 'center', fontWeight: 800, color: '#475569' }}>
                        {sub.percentage}%
                      </td>
          ...
          <div style={{ background: '#f8fafc', border: '1.5px solid #e2e8f0', borderRadius: 12, padding: 16, marginBottom: 20 }}>
            <h4 style={{ margin: '0 0 8px', fontSize: '.92rem', fontWeight: 900, color: '#0f172a' }}>
              🧠 التحليل والتفسير السيكومتري الإكلينيكي:
            </h4>
            <p style={{ margin: 0, fontSize: '.84rem', lineHeight: 1.7, color: '#334155' }}>
              {scoreResult.interpretation}
            </p>

            {assessment.notes && (
              <div style={{ marginTop: 12, paddingTop: 10, borderTop: '1px dashed #cbd5e1', fontSize: '.8rem', color: '#475569' }}>
          ...
          <div style={{ background: '#ffffff', border: '1.5px solid #bfdbfe', borderRadius: 12, padding: 16, marginBottom: 24 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              {scoreResult.recommendations.map((rec, i) => (
                <div key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: 8, fontSize: '.82rem', color: '#1e293b', lineHeight: 1.5 }}>
          ...
          {showItemDetails && (
            <div style={{ marginBottom: 24, border: '1px solid #cbd5e1', borderRadius: 10, padding: 16, background: '#fafafa' }}>
              <h4 style={{ fontSize: '.9rem', fontWeight: 900, color: '#0f172a', margin: '0 0 12px' }}>
          ...
          <div style={{ marginTop: 30, paddingTop: 16, borderTop: '2px solid #0f172a', display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', textAlign: 'center', gap: 16 }}>
            <div>
              <div style={{ fontSize: '.78rem', color: '#64748b', marginBottom: 4 }}>أخصائي العلاج الوظيفي والتكامل الحسي</div>
              <div style={{ fontSize: '.88rem', fontWeight: 800, color: '#0f172a' }}>{assessment.evaluator || 'الأخصائي المسؤول'}</div>
              <div style={{ marginTop: 24, fontSize: '.72rem', color: '#94a3b8' }}>التوقيع: ............................</div>
            </div>

            <div>
              <div style={{ fontSize: '.78rem', color: '#64748b', marginBottom: 4 }}>رئيس قسم التأهيل والتكامل الحسي</div>
              <div style={{ fontSize: '.88rem', fontWeight: 800, color: '#0f172a' }}>د. إشراف العلاج والتأهيل</div>
              <div style={{ marginTop: 24, fontSize: '.72rem', color: '#94a3b8' }}>التوقيع: ............................</div>
            </div>

            <div>
              <div style={{ fontSize: '.78rem', color: '#64748b', marginBottom: 4 }}>الختم والاعتماد الرسمي</div>
